# scripts/render.py  — airy/glass версия (fixed)
...
def accent_from_code(code: int):
    c = int(to_float(code))
    if c in (0,1):                      return (255, 194, 41)   # amber
    if c in (61,63,65,80,81,82,51,53):  return (74, 144, 226)   # blue
    if c in (71,73,75,77,85,86):        return (72, 209, 204)   # cyan
    if c in (95,96,99):                 return (171, 71, 188)   # purple
...
def drop_shadow(box_img, blur=16, shift=(0,8)):
    ...
    d.rounded_rectangle((blur,blur, blur+w-1, blur+h-1), radius=24, fill=SHADOW)
...
# FIX: чип рисуем на самом Image, а не через draw.im
def draw_chip(canvas: Image.Image, draw: ImageDraw.ImageDraw, pos, text, font, fg,
              bg_alpha=60, pad_x=18, pad_y=8, radius=16, anchor="mm"):
    x,y = pos
    # размер текста
    ...
    ImageDraw.Draw(box).rounded_rectangle((0,0,bw-1,bh-1), radius=radius,
                                          fill=(fg[0],fg[1],fg[2],bg_alpha))
    if anchor=="mm":
...
def draw_city(canvas: Image.Image, cx: int, top_y: int, city: dict, payload: dict, fonts: dict):
    ...
    d.text((cx, top_y+24+fonts["title_lead"]),
           ru_date_tomorrow(payload.get("date",""), payload.get("tz","")),
    ...
    gap = int(W*0.03)
    draw_chip(canvas, d, (cx-gap, top_y+int(card_h*0.83)), range_txt, fonts["chip"], ACC, anchor="rm")
    draw_chip(canvas, d, (cx+gap, top_y+int(card_h*0.83)), rain_txt,  fonts["chip"], ACC, anchor="lm")
...
def main():
    ...
    W,H = 1600, 900
    bg  = linear_gradient((W,H), top=(15,22,40), bottom=(9,14,28))
    img = bg.convert("RGBA")
